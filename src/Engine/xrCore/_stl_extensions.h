...
public:
	...
	template<class _Other> xalloc(const xalloc<_Other>&) noexcept {}
	template<class _Other> xalloc<T>& operator=(const xalloc<_Other>&) noexcept { return *this; }

	pointer allocate(size_type n, const void* = 0) {
		return static_cast<pointer>(xr_malloc(n * sizeof(T)));
	}
